...
curl -X POST http://localhost:8089/graphql \
  -H "Content-Type: application/json" \
  -d '{"query":"{ GetTaskByID(tenant_name: \"team-alpha\", task_id: \"TASK_ID_HERE\") { id status last_error shard_count created_at started_at finished_at shards { id script_name status output_oss_key output_sha256 last_error started_at finished_at } } }"}'
```
